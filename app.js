...
function calculateAudit() {
  ...
  const baseWage = rates[selectedArea][selectedGrade];
  const nightTotal =
  ...
  resultStatus.textContent = shortfall > 0 ? "Estimate" : "No difference";
...
function saveAudit() {
  ...
  localStorage.setItem("civicLedgerLastAudit", JSON.stringify(record));

  savedMessage.textContent = "Audit saved locally on this device.";
